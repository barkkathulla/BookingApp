...
class AddAppointments extends React.Component {
  ...
  save = (e) => {
    e.preventDefault();
    const { patientName, patientAge, gender, aptDate, aptFromTime, aptTime, aptNotes } = this.state;
    if (patientName !== "" && patientAge !== "" && gender !== "" && aptDate !== "" && aptFromTime !== "" && aptTime !== "" && aptNotes !== "") {
      let apt = {
        id: Date.now(),
        patientName: this.state.patientName,
        patientAge: this.state.patientAge,
        gender: this.state.gender,
        aptDate: this.state.aptDate + ' ' + this.state.aptFromTime,
  ...
  render() {
    ...
    return (
      <Card className="mt-4 mb-4 card-border" outline color="primary">
        <CardHeader style={styles} onClick={this.toggleBody}><i className="fas fa-plus"></i>Book for an Appointment</CardHeader>
        <CardBody style={displayBody} id="aptBody">
          ...
          <Form onSubmit={this.save}>
            ...
            <FormGroup>
              <Label for="aptDate">Visit Date</Label>
              <Input type="date" id="aptDate" value={this.state.aptDate} onChange={this.handleChange} />
            </FormGroup>
            <FormGroup>
              <Label for="aptFromTime">From Time</Label>
              <Input type="time" id="aptFromTime" value={this.state.aptFromTime} onChange={this.handleChange} />
            </FormGroup>
            <FormGroup>
              <Label for="aptTime">To Time</Label>
            ...
            <Alert color="danger" style={errors}>
              Please fill needed information
          </Alert>
            <Button type="submit" color="primary" block>Make an Appointment</Button>
          </Form>
        </CardBody>
      </Card >
    )
